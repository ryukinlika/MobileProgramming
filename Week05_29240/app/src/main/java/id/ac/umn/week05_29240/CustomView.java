package id.ac.umn.week05_29240;
...
public class CustomView extends View {
    ...
    private Path pathKu;
    private static Canvas canvasKu;
    ...
    public void gantiWarna(int r, int g, int b){
    ...
    private void mulaiSentuh(float x, float y){
        if(x <= 100 && y <= 100){
            canvasKu.drawColor(Color.WHITE);
            clear = true;
            invalidate();
        }
        else{
    ...
    private void geser(float x, float y){
        ...
        if(dx >= TOLERANSI_SENTUH || dy >= TOLERANSI_SENTUH){
            ...
            if(bentuk.equalsIgnoreCase("Path")){
                ...
                canvasKu.drawPath(pathKu, paintKu);
                pathX = x;
                pathY = y;
            }
            else{
    ...
    private void lepas(){
        if(!clear){
            if(bentuk.equalsIgnoreCase("Path")){
                ...
                canvasKu.drawRect(awalX, awalY, antarX, antarY, paintKu);
            }else if(bentuk.equalsIgnoreCase("Oval")){
                canvasKu.drawOval(awalX, awalY, antarX, antarY, paintKu);
            }else if(bentuk.equalsIgnoreCase("Line")){
                canvasKu.drawLine(awalX, awalY, antarX, antarY, paintKu);
            }
        }
